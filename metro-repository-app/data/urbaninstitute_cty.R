...
list_auto <- names(auto)
list_med <- names(med)
list_student <- names(student)
list_overall <- names(overall)

rm(oldw)

debt <- debt_overall

common <- intersect(names(debt), names(debt_overall))
debt <- merge(debt, debt_overall, by=common, all.x=TRUE)
rm(common)

common <- intersect(names(debt), names(debt_auto))
debt <- merge(debt, debt_auto, by=common, all.x=TRUE)
rm(common)

common <- intersect(names(debt), names(debt_med))
debt <- merge(debt, debt_med, by=common, all.x=TRUE)
rm(common)

common <- intersect(names(debt), names(debt_student))
debt <- merge(debt, debt_student, by=common, all.x=TRUE)
rm(common)
